...
class ContextVLM:
    ...
    def run_inference(
        ...
        **kwargs,
        ...
        api_type, model_name = parts[0], parts[-1]
        completion_ids = []
        prompt_ids = []
        attention_mask = []
        if "qwen" in model_name or "molmo2" in model_name:
            ...
            response = self.client.get_response(prompt=query, image_urls=media_paths, model=model_name, temperature=temperature)[0]
            print(f"[DEBUG] GPT raw response (first 1000 chars):\n{str(response)[:1000]}")
        ...
        if parse_json:
            ...
            if _json_parse_failed:
                print(f"[DEBUG] JSON parse failed. Raw response (first 1000 chars):\n{str(response)[:1000]}")
                if return_ids:
                    pass  # keep response as raw string for token logging
                else:
                    response = {
                        "video_context": "",
                        "query_intent": "",
                        "final_answer": None,
                        "recommended_tools": {"needed": False, "tool_calls": []},
                    }
    ...
    def get_context_vlm_prompt(
        ...
        if "qwen" in self.api_type or "molmo2" in self.api_type:
            ...
            if self.use_video and video_duration > 0:
                gpt_max_frames = int(os.environ.get("GPT_MAX_FRAMES", "16"))
                media_paths = extract_frames(
                    video_path,
                    0,
                    video_duration,
                    num_frames=min(gpt_max_frames, max(2, int(video_duration))),
                )
                media_type = "image"
            query = (
                CONTEXT_VLM_PROMPT.replace("<<<query>>>", query)
    ...
    def get_video_duration(self, video_path: str) -> float:
        ...
        if not fps:
            print(f"Warning: Could not read FPS from video: {video_path}, defaulting duration to 0")
            return 0
        return int(cap.get(cv2.CAP_PROP_FRAME_COUNT) / fps)